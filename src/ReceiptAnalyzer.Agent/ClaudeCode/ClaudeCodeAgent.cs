...
namespace ReceiptAnalyzer.Agent.ClaudeCode;

/// <summary>
/// Calls headless Claude Code (<c>claude -p</c>) via the host-side HTTP bridge
/// (<c>ReceiptAnalyzer.Bridge</c>) instead of a hosted LLM API. Chosen so receipt analysis rides a
/// Max subscription at zero marginal API cost. Vision input goes via a shared-tmp file the bridge's
/// host-side <c>claude</c> process reads directly (no image upload over HTTP); everything else is a
/// plain prompt. Prompts are kept at parity with <see cref="Claude.ClaudeAgent"/>.
/// </summary>
public sealed class ClaudeCodeAgent : IAnalysisAgent
{
    ...
    public ClaudeCodeAgent(HttpClient http, IOptions<AgentOptions> options, ILogger<ClaudeCodeAgent> logger)
    {
        _http = http;
    ...
    {
        var tmpDir = ResolveTmpDir();
        Directory.CreateDirectory(tmpDir);
        var tmpPath = Path.Combine(tmpDir, $"{Guid.NewGuid():N}{ExtensionFor(mediaType)}");
        await File.WriteAllBytesAsync(tmpPath, imageBytes, ct);
        try
        {
            var prompt = _rules.Value + "\n\n" + ExtractionSystemPrompt + "\n\n" + ExtractionUserPrompt;
    ...
    public async Task<PriceCheckResult> PriceCheckAsync(IReadOnlyList<BrandedItemForCheck> items, CancellationToken ct, string? hint = null)
    {
        ...
        var prompt = PriceCheckSystemPrompt + $$"""
        ...
        var json = await RunAsync(prompt, imagePath: null, "price-check", ct, allowedTools: ["WebSearch"]);
        var parsed = JsonSerializer.Deserialize<PriceCheckRaw>(json, JsonOptions)
            ?? throw new InvalidOperationException("Cannot parse bridge price check JSON.");
    ...
    /// <summary>
    /// Posts one <c>/agent/run</c> request to the bridge and reports the returned token usage under
    /// the bridge-reported model (falling back to the configured model). The bridge shells out to
    /// the host-side <c>claude -p</c> CLI and returns its parsed JSON envelope.
    /// </summary>
    private async Task<string> RunAsync(string prompt, string? imagePath, string stage, CancellationToken ct, IReadOnlyList<string>? allowedTools = null)
    {
        var body = new JsonObject { ["prompt"] = prompt };
        if (imagePath is not null)
            body["imagePath"] = imagePath;
        body["model"] = _options.ClaudeCode.Model;
        body["timeoutSeconds"] = _options.ClaudeCode.TimeoutSeconds;
        if (allowedTools is { Count: > 0 })
            body["allowedTools"] = new JsonArray(allowedTools.Select(t => (JsonNode)t).ToArray());

        var bridgeKey = Environment.GetEnvironmentVariable(_options.ClaudeCode.BridgeKeyEnvVar);
        var uri = new Uri(new Uri(_options.ClaudeCode.BridgeUrl.TrimEnd('/') + "/"), "agent/run");

        using var req = new HttpRequestMessage(HttpMethod.Post, uri) { Content = JsonContent.Create(body) };
        if (!string.IsNullOrEmpty(bridgeKey))
            req.Headers.Add("X-BRIDGE-KEY", bridgeKey);

        using var resp = await _http.SendAsync(req, ct);
        var raw = await resp.Content.ReadAsStringAsync(ct);
        if (!resp.IsSuccessStatusCode)
        {
            _logger.LogError("Bridge call failed: {Status} {Body}", resp.StatusCode, raw);
            throw new HttpRequestException($"Bridge returned {(int)resp.StatusCode}: {raw}");
        }

        var doc = JsonNode.Parse(raw) ?? throw new InvalidOperationException("Empty bridge response.");
        var isError = (bool?)doc["isError"] ?? false;
        var resultText = (string?)doc["resultText"];
        if (isError || string.IsNullOrWhiteSpace(resultText))
            throw new InvalidOperationException($"Bridge agent run reported an error: {resultText ?? "(empty)"}");

        var model = (string?)doc["model"] ?? _options.ClaudeCode.Model;
        var input = (int?)doc["inputTokens"] ?? 0;
        var output = (int?)doc["outputTokens"] ?? 0;
        _logger.LogInformation("Claude Code {Stage} usage: input={Input} output={Output} model={Model}", stage, input, output, model);
        UsageReporter.Report(new StageUsage(stage, model, input, output, 0, 0));

        return ExtractJson(resultText);
    }

    /// <summary>
    /// Directory for temp receipt images, host-visible via the bridge's path map. Falls back to the
    /// machine temp folder when unconfigured (e.g. running the agent standalone in a test).
    /// </summary>
    private string ResolveTmpDir() =>
        string.IsNullOrWhiteSpace(_options.ClaudeCode.TmpDir)
            ? Path.Combine(Path.GetTempPath(), "receipt-analyzer-bridge-tmp")
            : _options.ClaudeCode.TmpDir;
...
Apply these rules strictly (from the Shopping Agent rules above):
...
The item names provided have already been expanded to real product names — use the WebSearch tool to find the current price for each at major UK supermarkets: Tesco, Sainsbury's, Asda, Morrisons, Waitrose, Ocado, Aldi, Lidl.
...
Use trolley.co.uk as a reference source, and cite the exact page as sourceUrl.
